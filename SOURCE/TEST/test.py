...
class DMSE(tf.keras.losses.Loss):
  ...
  def __call__(self, y_true, y_pred, sample_weight=None):
    ...
    loss = tf.cond(
              tf.equal(tf.size(loss), 0), 
              lambda : tf.constant(0.0), lambda: tf.reduce_mean(loss)
...
burned_mse = WildFirePhysicsLoss(
            constraint = 0,
            burned = 1,
            unburned = 0, 
            firemetrics =0 
        )
unburned_mse = WildFirePhysicsLoss(
            constraint = 0,
            burned = 0,
            unburned = 1, 
            firemetrics =0 
        )
ROS_mse = WildFirePhysicsLoss(
            constraint = 0,
            burned = 0,
            unburned = 0, 
            firemetrics = 1 
...
model = build_model()
model = model.load_weights(ps.path.join(save_dir, 'pgcl.ckpt'))
mses = np.zeros((len(test_datasets), 7))
for i in range(len(test_datasets)):
    pred =  model.predict(test_datasets[i])
    mses[ i,0] = mse(test_ys[i].ravel(), pred.ravel()) 
    mses[i,1], mses[i, 2] = test_datasets[i][-1,0,0,0,2], test_datasets[i][-1,0,0,0,1] # wind speed, direction
    mses[i, 3] = burned_mse(test_ys[i], pred)
    mses[i, 4] = unburned_mse(test_ys[i], pred)
    mses[i, 5] = ROS_mse(test_ys[i], pred) 
    mses[i, 6] = dmse_loss(test_ys[i], pred) 
print('Average MSE: {}, Burned: {}, Unburned: {}, ROS: {}, VMSE: {}'.format(np.mean(mses[:, 0]), np.mean(mses[:, 3]), np.mean(mses[:, 4]), np.mean(mses[:, 5]), np.mean(mses[:, 6])))
